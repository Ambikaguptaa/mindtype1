...
def simulate_session(user_baseline_speed, user_baseline_var, label, n=None):
    """label: 0 = baseline/calm, 1 = strained.
    n: number of keystrokes to simulate (defaults to a random passage-like
    length so the training data matches the real 5-passage capture flow)."""
    if n is None:
        n = int(RNG.integers(MIN_KEYSTROKES, MAX_KEYSTROKES + 1))
