...
from features import compute_user_baselines, build_sequences, fit_scaler_masked, apply_scaler, FEATURES, SEQ_LEN
...
def main():
    ...
    X, y, session_ids, lengths = build_sequences(df, baselines)
    print(f"Built {X.shape[0]} sessions, max sequence length {X.shape[1]}, {X.shape[2]} features/step "
          f"(real lengths range {lengths.min()}-{lengths.max()})")

    X_train, X_test, y_train, y_test, len_train, len_test, sid_train, sid_test = train_test_split(
        X, y, lengths, session_ids, test_size=0.2, random_state=42, stratify=y
    )

    mean, std = fit_scaler_masked(X_train, len_train)
    X_train_s = apply_scaler(X_train, mean, std, len_train)
    X_test_s = apply_scaler(X_test, mean, std, len_test)
